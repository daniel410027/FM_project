...
creates composite scores for profitability and growth, sorts stocks into deciles, 
...
def load_make_data(file_path='database/make2.csv'):
    """
    ...
    try:
        ...
        # Filter data to include only periods after 200000
        df_filtered = df[df['年月'] > 200000].copy()
        print(f"Filtered data shape (年月 > 200000): {df_filtered.shape}")
...
def standardize_ratios_by_period(df):
    ...
    # Level ratio columns to standardize
    level_ratio_cols = ['gpoa', 'roe', 'cfoa', 'gmar', 'acc']
    
    # Delta ratio columns to standardize
    delta_ratio_cols = ['delta of gpoa', 'delta of roe', 'delta of cfoa', 
                        'delta of gmar', 'delta of acc']
    
    # Check if all required columns exist
    required_columns = ["證券代碼", "年月", "公司名稱", "rtn"] + level_ratio_cols + delta_ratio_cols
    ...
    # Standardize level ratios by period (年月)
    print("Standardizing level financial ratios by period...")
    
    # Create standardized ratio columns with 'z_' prefix
    for ratio in level_ratio_cols:
        result_df[f'z_{ratio}'] = np.nan
    
    # Create standardized delta ratio columns with 'z_' prefix
    for ratio in delta_ratio_cols:
        result_df[f'z_{ratio}'] = np.nan
    ...
    for period in result_df['年月'].unique():
        ...
        for ratio in delta_ratio_cols:
            ...
            if len(valid_values) > 0:
                ...
                # If all values are NaN, keep z-scores as NaN
                result_df.loc[period_mask, f'z_{ratio}'] = np.nan
    
    # Create profitability score (sum of standardized level ratios)
    print("Creating profitability score...")
    result_df['profitability'] = sum(result_df[f'z_{ratio}'] for ratio in level_ratio_cols)
    
    # Create growth score (sum of standardized delta ratios)
    print("Creating growth score...")
    result_df['growth'] = sum(result_df[f'z_{ratio}'] for ratio in delta_ratio_cols)
    
    # Create combined score (you can adjust the weights as needed)
    print("Creating combined score...")
    result_df['score'] = result_df['profitability'] + result_df['growth']
...
def analyze_returns_by_group(df):
    ...
    # Drop rows with NaN returns or groups, and filter for 年月 > 200000
    analysis_df = df[(df['年月'] > 200000)].dropna(subset=['rtn', 'group']).copy()
    
    print(f"Analyzing returns by group (年月 > 200000)...")
...
def save_sorted_data(df, output_path='database/sort2.csv'):
    """
...
def save_analysis_results(group_stats, hl_summary, hl_df=None, industry_effects=None,
    ...
    try:
        ...
        group_stats.to_csv(output_path / 'group_stats2.csv', index=False, encoding='utf-8')
        print(f"Group statistics saved to {output_path / 'group_stats2.csv'}")
        
        # Save H-L summary
        hl_summary.to_csv(output_path / 'hl_summary2.csv', index=False, encoding='utf-8')
        print(f"H-L summary saved to {output_path / 'hl_summary2.csv'}")
        
        # Save H-L returns by period if available
        if hl_df is not None:
            hl_df.to_csv(output_path / 'hl_returns2.csv', index=False, encoding='utf-8')
            print(f"H-L returns saved to {output_path / 'hl_returns2.csv'}")
            
        # Save industry fixed-effects analysis if available
        if industry_effects is not None and not industry_effects.empty:
            industry_effects.to_csv(output_path / 'industry_effects2.csv', index=False, encoding='utf-8')
            print(f"Industry fixed-effects analysis saved to {output_path / 'industry_effects2.csv'}")
    except Exception as e:
        print(f"Error saving analysis results: {e}")


def run_sort_process(input_file='database/make2.csv', output_file='database/sort2.csv'):
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Sort and analyze financial data with profitability and growth scores')
    parser.add_argument('--input', default='database/make2.csv', help='Input CSV file path')
    parser.add_argument('--output', default='database/sort2.csv', help='Output CSV file path')
