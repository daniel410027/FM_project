...
def create_financial_ratios(df):
    ...
    # o = -1 * (6.03 * 負債總額 / 資產總額 - 1.43 * 來自營運之現金流量 / 資產總額 - 2.37 * 稅後淨利率 - 1.83 * 來自營運之現金流量 / 負債總額)
    # 注意：需要處理除以零的情況
    ...
    delta_cols = ['gpoa', 'roe', 'cfoa', 'gmar', 'acc']  # acc is included in delta calculations
...
def save_processed_data(df, output_path='database/make3.csv'):
    """
...
def run_make_process(input_file='database/first_backfill.csv', output_file='database/make3.csv'):
    """
    ...
    if backfilled_df is not None:
        ...
        if processed_df is not None:
            ...
            ratio_cols = ['gpoa', 'roe', 'cfoa', 'gmar', 'acc', 
                         'delta of gpoa', 'delta of roe', 'delta of cfoa', 'delta of gmar',
                         'bab', 'lev', 'o', 'z', 'evol',
                         'rtn']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', default='database/make3.csv', help='Output CSV file path')
