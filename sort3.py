...
def load_make_data(file_path='database/make3.csv'):
    """
...
def save_sorted_data(df, output_path='database/sort3.csv'):
    """
...
def save_analysis_results(group_stats, hl_summary, hl_df=None, industry_effects=None,
    ...
    try:
        ...
        group_stats.to_csv(output_path / 'group_stats3.csv', index=False, encoding='utf-8')
        print(f"Group statistics saved to {output_path / 'group_stats3.csv'}")
        
        # Save H-L summary
        hl_summary.to_csv(output_path / 'hl_summary3.csv', index=False, encoding='utf-8')
        print(f"H-L summary saved to {output_path / 'hl_summary3.csv'}")
        
        # Save H-L returns by period if available
        if hl_df is not None:
            hl_df.to_csv(output_path / 'hl_returns3.csv', index=False, encoding='utf-8')
            print(f"H-L returns saved to {output_path / 'hl_returns3.csv'}")
            
        # Save industry fixed-effects analysis if available
        if industry_effects is not None and not industry_effects.empty:
            industry_effects.to_csv(output_path / 'industry_effects3.csv', index=False, encoding='utf-8')
            print(f"Industry fixed-effects analysis saved to {output_path / 'industry_effects3.csv'}")
    except Exception as e:
        print(f"Error saving analysis results: {e}")


def run_sort_process(input_file='database/make3.csv', output_file='database/sort3.csv'):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', default='database/make3.csv', help='Input CSV file path')
    parser.add_argument('--output', default='database/sort3.csv', help='Output CSV file path')
